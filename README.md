...
Gain Index (delta) against empirically computed Bertrand-Nash and monopoly
bounds (Calvano et al., 2020).

**Students:** Milan Sazdov (SV21/2023), Lazar Sazdov (SV25/2023)
**Course:** Osnovi racunarske inteligencije, FTN, University of Novi Sad

## Results

120 training runs (6 experiments x 20 seeds x 500k steps). Delta = 0 means
competitive (Nash) profits, delta = 1 full cartel; the collusion threshold
tested is delta > 0.15.

| Experiment | Setup | mean delta (std) | mean price / median |
|---|---|---|---|
...
| E6 | D3QN, 2 agents, collusion-friendly settings | -0.99 (0.19) | 1.09 |

- **H1 (DQN collusion): not supported** (one-sided t-test, p = 1.0). DQN
  agents beat every static baseline by at least 15% yet price near the
  market median, below Nash profits. In a realistic, data-calibrated
  market with a practical training budget, collusion is not an automatic
  emergent property; the classic results arise after millions of episodes
  in stylized markets.
- **H2 (collusion depends on the algorithm): supported** (Kruskal-Wallis
  H = 51.2, p = 7.7e-12), with the observed ordering reversing the
  literature's expectation. PPO is the only algorithm with profits above
  the competitive level, achieved through tacit market division: one
  ...
  Under a strictly greedy replay the E3 delta falls to -0.15, disclosed
  and read as confirmation of the mechanism: the stochastic policy itself
  sustains the split.
- **H3 (fewer agents collude more): inconclusive by design.** The formal
  delta test is not interpretable here: the 4-agent market's modeled
  demand is nearly price-inelastic, its computed Nash sits at the price
  cap, and alternative profit normalizations yield opposite verdicts. The
...
- **E6 (robustness ablation): the H1 verdict survives collusion-friendly
  settings.** Rerunning E1 with discount 0.99, exploration decaying to
  0.01 and a 20k replay buffer yields delta -0.99 (0.19) with no seed
  above Nash. The levers that enable collusion in the tabular literature
  only destabilize learning here, so the absence of collusion is a
  property of the market and budget, not of one hyperparameter choice.

Per-seed numbers live in `results/evaluation/`, figures come from
`scripts/make_figures.py`, working notes and every specification deviation
in `docs/report_notes.md`. The written report is `results.pdf` and the
defense slides `prezentacija.pdf` (both in Serbian, sources included); an
English overview deck is `docs/presentation.html`.
...
- Two seasonal snapshot pairs (May to June 2026, September to October
  2025) are used because within a single pair, booking lead time and stay
  date are perfectly collinear; a second season separates them.
...
- Cross-sectional prices understate the causal price response (expensive
  flats are also nicer flats, so the raw revenue curve peaked at the
  price cap). The simulation applies P(r) * exp(beta * (r - 1)) with
  beta = -0.425, calibrated so the revenue peak lands at ratio 1.1. A
  power-law correction cannot produce an interior revenue peak here; the
  exponential form matches logit demand, where elasticity grows with
  price, as in Calvano et al.
...
times the cluster median with boundary hits logged.

- PettingZoo over plain Gymnasium because simultaneous price-setting is
  multi-agent by nature; over RLlib because a custom loop keeps one
...
- Both markets are real listing groups. The 4-agent market is a
  co-located block that one professional host runs in reality; E2
  simulates it counterfactually with four independent agents, a what-if
  for the scenario where ownership fragments and pricing tools are
  available to anyone. Agent independence is the experimental treatment,
...
- **PPO** is a minimal custom implementation (about 200 lines, clipped
  surrogate plus GAE) rather than Stable-Baselines3: SB3 does not
  natively drive PettingZoo multi-agent loops, and wrapper stacks would
  split the code path the other agents share.
...
use the same demand model and reward normalization as training, so delta
is internally consistent. Analytic bounds (as in logit-demand papers) are
...
docs/               report_notes.md, report_figures/, presentation.html
results.tex/.pdf    written report (Serbian)
prezentacija.tex/.pdf   defense slides (Serbian, Beamer)
```
...
python3 scripts/download_data.py      # Inside Airbnb snapshots (about 130 MB)
python3 scripts/preprocess.py         # labels, clusters, features, writes parquet
...
Smoke configs in `configs/smoke/` verify the pipeline in minutes. The
LaTeX documents compile with any pdflatex or xelatex distribution from the
repo root, with figures read from `docs/report_figures/`.
...
Calvano, Calzolari, Denicolo, Pastorello (2020), *AI, Algorithms and
...
(2015), Nature 518. Sutton and Barto (2018), *Reinforcement Learning*.
